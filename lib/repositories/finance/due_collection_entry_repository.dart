...
class DueCollectionEntryRepository {
  ...
  Future<DueCollectionSaveResult> saveCollection({
    ...
    try {
      return await _db.transaction(() async {
        ...
        final currentDue = BillDuePolicy.financeDue(bill);
  ...
  List<DueCollectionBillModel> _mapDueRows(List<TypedResult> rows) {
    ...
    for (final row in rows) {
      ...
      final due = BillDuePolicy.financeDue(bill);
      if (due <= BillDuePolicy.financeVisibilityTolerance) continue;
  ...
  Future<String> _insertCashReceipt({
    ...
    return _retryTransactionInsert((attempt) async {
      final txnId = await _numbering.nextCashTransactionId(offset: attempt);
  ...
  Future<String> _insertBankReceipt({
    ...
    return _retryTransactionInsert((attempt) async {
      final txnId = await _numbering.nextBankTransactionId(offset: attempt);
  ...
  Future<String> _retryTransactionInsert(
    Future<String> Function(int attempt) insert,
  ) async {
    Object? lastError;
    ...
    throw StateError('Could not allocate a unique finance transaction ID: '
        '$lastError');
  }
