...
class PosCheckoutRepository {
  ...
  Future<void> updateSale({
    ...
    await _db.transaction(() async {
      ...
      await _restoreLinkedStock(existingItems);
  ...
  Future<void> _restoreLinkedStock(List<BillItem> billItems) async {
    final stockUsage = <int, int>{};
    ...
    for (final item in billItems) {
      ...
      stockUsage.update(
        stockItemId,
        (current) => current + _unitsForQuantity(item.quantity),
        ifAbsent: () => _unitsForQuantity(item.quantity),
      );
    }
