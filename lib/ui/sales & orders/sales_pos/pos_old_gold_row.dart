        ...
        return MouseRegion(
          ...
          child: AnimatedContainer(
            ...
            child: Row(
              ...
              children: [
                Expanded(flex: 1, child: _buildSNo(widget.index + 1, metalColor)),
                ...
                Expanded(
                  ...
                  child: PosAtomicTextField(
                    ...
                    hint: "0.000", 
                    isNumber: true, 
                    focusNode: isWholesale ? widget.item.firstFieldFocus : null
                  ),
                ),
                const SizedBox(width: 6),
                Expanded(
                  flex: isWholesale ? 4 : 2,
                  child: PosAtomicTextField(controller: widget.item.lessCtrl, hint: "0.000", isNumber: true),
                ),
                const SizedBox(width: 6),
                Expanded(
                  flex: isWholesale ? 4 : 2,
                  child: _buildAutoCell(value: widget.item.netWt.toStringAsFixed(3), color: metalColor, align: TextAlign.left),
                ),
                const SizedBox(width: 6),
                Expanded(flex: isWholesale ? 4 : 2, child: _buildCleanPurityInput(metalColor, isWholesale)), 
                const SizedBox(width: 6),
                Expanded(
                  flex: isWholesale ? 4 : 2,
                  child: _buildAutoCell(value: widget.item.fineWt.toStringAsFixed(3), color: metalColor, align: TextAlign.left, isBold: true),
                ),
                const SizedBox(width: 6),
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<MetalType>(
          ...
          items: MetalType.values.map((type) => DropdownMenuItem<MetalType>(value: type, child: Text(type.displayName.toString().toUpperCase()))).toList(),
    ...
    return Container(
      ...
      child: TextFormField(
        ...
        decoration: InputDecoration(
          ...
          hintStyle: SalesPosStyles.subTitleMuted.copyWith(color: metalColor.withOpacity(0.50)),
        ),
      ),
    );
  }

  Widget _buildAutoCell({required String value, required Color color, TextAlign align = TextAlign.center, bool isBold = false}) {
