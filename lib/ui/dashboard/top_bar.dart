...
class _TopBarState extends State<TopBar> with TickerProviderStateMixin {
  ...
  // 🔥 SENIOR UPGRADE: Debounce Timer (Performance Saver)
  Timer? _debounce;
  ...
  void initState() {
    ...
    _profileGlowController = AnimationController(vsync: this, duration: const Duration(seconds: 3))..repeat(reverse: true);
  ...
  void _updateSearchResults(String query) async {
    ...
    List<SearchResult> results = await widget.repository.searchUniversal(query, selectedScope, limit: 10);
      ...
      child: BackdropFilter(
        ...
        child: Container(
          decoration: BoxDecoration(
            color: TopBarColors.background.withOpacity(0.95),
          ...
          child: isLoading
              ? const SizedBox(height: 100, child: Center(child: CircularProgressIndicator(color: TopBarColors.accentGold)))
              : results.isEmpty
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          leading: Container(
                            ...
                            decoration: BoxDecoration(
                              color: TopBarColors.accentGold.withOpacity(0.15),
                              shape: BoxShape.circle,
                            ),
                            child: Icon(_getIconForType(item.type), color: TopBarColors.accentGold, size: 18),
                          ),
                          title: Text(item.title, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
                          subtitle: Text(item.subtitle, style: const TextStyle(color: TopBarColors.textSecondary, fontSize: 12)),
  ...
  Widget _buildNoResultsFound() {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(Icons.search_off, color: Colors.white.withOpacity(0.3)),
          const SizedBox(width: 10),
          Text("No results found", style: TextStyle(color: Colors.white.withOpacity(0.5))),
        ],
      ),
    );
  }
  ...
  IconData _getIconForType(String type) {
    switch (type.toLowerCase()) {
      ...
      case 'loan': return Icons.diamond_outlined;
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Row(
            ...
            children: [
              const Text("Notifications", style: TextStyle(color: Colors.white, fontSize: 18)),
              TextButton(
                onPressed: () {
                  widget.repository.markAllRead(widget.currentUser.role);
                  Navigator.pop(context); // Close dialog
                },
                child: const Text("Mark all read", style: TextStyle(color: TopBarColors.accentGold, fontSize: 12)),
              )
            ],
          ),
          content: SizedBox(
              ...
              builder: (context, snapshot) {
                ...
                return ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Icon(Icons.notifications, color: item.isRead ? Colors.grey : TopBarColors.accentGold),
  ...
  void _showProfileMenu() async {
    final RenderBox? button = _profileKey.currentContext?.findRenderObject() as RenderBox?;
    ...
    final String? selectedValue = await showMenu<String>(
      ...
      elevation: 10,
      items: [
        const PopupMenuItem(value: "profile", child: Row(children: [Icon(Icons.person, color: TopBarColors.accentGold), SizedBox(width: 10), Text("My Profile", style: TextStyle(color: Colors.white))])),
        const PopupMenuItem(value: "logout", child: Row(children: [Icon(Icons.logout, color: TopBarColors.notificationRed), SizedBox(width: 10), Text("Logout", style: TextStyle(color: Colors.white))])),
      ],
    );
      ...
      return ClipRRect(
        child: BackdropFilter(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: TopBarColors.background.withOpacity(0.85), 
                    ...
                    child: isDesktop 
                      ...
                      : InkWell( 
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(color: TopBarColors.accentGold.withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
                            child: const Icon(TopBarIcons.search, color: TopBarColors.accentGold),
                          ),
                        ),
                  )
                ), 
    ...
    return Row(
      children: [
        Container(
          ...
          child: const Icon(TopBarIcons.dashboard, color: TopBarColors.accentGold, size: 22),
        ),
        ...
        Column(
          ...
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Text("DASHBOARD", style: TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 16, letterSpacing: 1.5)),
            const SizedBox(height: 4),
            Row(
              children: [
                FadeTransition(
                  opacity: _pulseAnimation,
                  child: Container(width: 8, height: 8, decoration: const BoxDecoration(color: TopBarColors.systemOnline, shape: BoxShape.circle, boxShadow: [BoxShadow(color: TopBarColors.systemOnline, blurRadius: 6)])),
                ),
                const SizedBox(width: 8),
                const Text("System Online", style: TextStyle(color: TopBarColors.textSecondary, fontSize: 11, fontWeight: FontWeight.w600)),
              ],
            )
          ],
        )
          ...
          children: [
            Material(
              ...
              child: PopupMenuButton<SearchScope>(
                ...
                itemBuilder: (context) => SearchScope.values.map((scope) => PopupMenuItem(
                  value: scope, 
                  child: Text(scope.label, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))
                )).toList(),
                child: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 16),
                  child: Row(
                    children: [
                      Text(selectedScope.label, style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.w800, fontSize: 13)),
            ...
            Material(
              ...
              child: InkWell(
                ...
                child: Container(
                  ...
                  child: const Icon(TopBarIcons.search, color: Colors.black, size: 22),
                ),
              ),
            ),
          ],
        ...
        if (isDesktop) ElevatedButton(
          ...
          child: Row(
            ...
            children: const [
              ...
              SizedBox(width: 8),
              Text("NEW SALE", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w900, fontSize: 12)),
            ],
          ),
        ),
          ...
          child: AnimatedBuilder( 
            ...
            builder: (context, child) {
              ...
              if (_bellController.isAnimating) {
                offset = 4 * (1 - _bellController.value) * (0.5 - (0.5 - _bellController.value).abs()).sign; 
              }
            ...
            child: Stack(
              children: [
                Container(
                  ...
                  child: const Icon(TopBarIcons.notification, color: Colors.white70),
                ),
                if (_cachedNotifications.any((n) => !n.isRead)) Positioned(
                  top: 10, right: 12,
                  child: Container(width: 10, height: 10, decoration: BoxDecoration(color: TopBarColors.notificationRed, shape: BoxShape.circle, border: Border.all(color: TopBarColors.background, width: 2))),
                )
              ],
            ),
          ),
  ...
  Widget _buildProfileCard(BuildContext context) {
    ...
    Color primaryColor = isOwner ? TopBarColors.accentGold : const Color(0xFF64B5F6);
          ...
          return InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: Colors.white.withOpacity(0.1)),
                boxShadow: [
                  BoxShadow(color: primaryColor.withOpacity(glowOpacity), blurRadius: 12, spreadRadius: spread)
                ]
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  SizedBox(
                    ...
                    child: Stack(
                      ...
                      children: [
                        Container(decoration: BoxDecoration(shape: BoxShape.circle, gradient: SweepGradient(colors: [primaryColor, Colors.transparent]))),
                        CircleAvatar(radius: 17, backgroundColor: const Color(0xFF1A1D24), child: CircleAvatar(radius: 15, backgroundColor: primaryColor, child: Text(widget.currentUser.initials, style: const TextStyle(color: Colors.black, fontSize: 11, fontWeight: FontWeight.w900)))),
                        Positioned(bottom: 1, right: 1, child: Container(width: 10, height: 10, decoration: BoxDecoration(color: const Color(0xFF00E676), shape: BoxShape.circle, border: Border.all(color: const Color(0xFF1A1D24), width: 1.5))))
                      ],
                    ),
                  ),
                  ...
                  Column(
                    ...
                    children: [
                      Text(widget.currentUser.name, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700, letterSpacing: 0.5, height: 1.0)),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          boxShadow: [BoxShadow(color: primaryColor.withOpacity(0.4), blurRadius: 4)]
                        ),
                        child: Text(roleDisplay, style: const TextStyle(color: Colors.black87, fontSize: 9, fontWeight: FontWeight.w900, letterSpacing: 0.5, height: 1.0)),
                      ),
                    ],
                  ),
                  const SizedBox(width: 14),
