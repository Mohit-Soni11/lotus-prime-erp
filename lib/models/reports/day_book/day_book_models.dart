...
class CashInflow {
  ...
  double get retailSalesTotal =>
      gstSales.finalAmount + nonGstSales.totalAmount;

  double get manualIncomeTotal =>
      advance + orderDelivery + girviReturn +
      loanReceived + interestRec + miscIncome + otherIncome;
...
class CashOutflow {
  ...
  double get operationalExpenses =>
      shopRent + staffSalary + electricity + maintenance +
      advertising + transport + bankCharges + govtFees;

  double get total =>
      shopRent + staffSalary + electricity + purchasePayment +
      girviGiven + maintenance + advertising + transport +
      bankCharges + govtFees + miscExpense + otherExpense;
}
...
class MetalInflow {
  ...
  MetalWeight get total =>
      karigarFinishedGoods + girviSecurityDeposit +
      urdPurchase + salesReturnReversal;
}
