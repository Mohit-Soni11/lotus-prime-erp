...
class TaxGstLogic extends ChangeNotifier {
  ...
  List<String> get taxpayerTypes => TaxpayerType.values.map((e) => e.displayName).toList();

  // --- HSN DATA (Mocked for now, usually comes from API) ---
  final List<Map<String, dynamic>> hsnList = [
    {"title": "Gold Jewellery", "label": "HSN CODE", "code": "71131910", "rate": "3%"},
    {"title": "Silver Articles", "label": "HSN CODE", "code": "71131120", "rate": "3%"},
    {"title": "Diamond / Stones", "label": "HSN CODE", "code": "71023910", "rate": "3%"},
  ];
  ...
  List<FocusNode> _validateGstSection() {
    ...
    if (TaxGstValidators.validateGstin(gstinCtrl.text) != null) errors.add(gstinFocus);
    if (TaxGstValidators.validateLegalName(legalNameCtrl.text) != null) errors.add(legalNameFocus);
    if (TaxGstValidators.validateDate(regDateCtrl.text, "Registration Date") != null) errors.add(regDateFocus);
    return errors;
  }

  List<FocusNode> _validateBisSection() {
    List<FocusNode> errors = [];
    if (TaxGstValidators.validateBisLicense(bisLicCtrl.text) != null) errors.add(bisLicFocus);
    if (TaxGstValidators.validateDate(validFromCtrl.text, "Valid From") != null) errors.add(validFromFocus);
    if (TaxGstValidators.validateDate(validUptoCtrl.text, "Valid Upto") != null) errors.add(validUptoFocus);
    return errors;
  }
  ...
  Future<bool> saveSection(String sectionId) async {
    ...
    if (sectionId == 'gst') gstSectionState = SectionEditState.locked;
