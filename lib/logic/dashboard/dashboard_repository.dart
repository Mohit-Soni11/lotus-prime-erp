...
class DashboardRepository {
  ...
  final _notificationController = StreamController<List<NotificationItem>>.broadcast();

  Stream<List<NotificationItem>> get notificationsStream => _notificationController.stream;
  ...
  void loadNotifications(String role) async {
    try {
      ...
      final uiList = dbList.map((row) => NotificationItem(
        id: row.id,
        type: row.type,
        title: row.title,
        desc: row.desc,
  ...
  Future<CustomerStatsModel> fetchCustomerStats() async {
    try {
      ...
      final String formattedTime = "${today.hour.toString().padLeft(2, '0')}:${today.minute.toString().padLeft(2, '0')}";
  ...
  // 6. SEARCH LOGIC
  // ==========================================
  Future<List<SearchResult>> searchUniversal(String query, SearchScope scope, {int limit = 10}) async {
    ...
    // 1. Customer Search
    if ([SearchScope.all, SearchScope.customer, SearchScope.mobile].contains(scope)) {
    ...
    if ([SearchScope.all, SearchScope.invoice].contains(scope)) {
      ...
      results.addAll(bills.map((b) => SearchResult(
        id: b.id.toString(),
        title: b.billNo,
        subtitle: "Amt: ₹${b.totalAmount.toStringAsFixed(2)}",
        type: "Invoice"
      )));
    }
    ...
    if ([SearchScope.all, SearchScope.loan].contains(scope)) {
      final loans = await (_db.select(_db.loans)
        ..where((tbl) => tbl.loanNo.contains(term) | tbl.itemDesc.contains(term))
        ..limit(5)
      ).get();
