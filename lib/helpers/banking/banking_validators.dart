...
class BankingValidators {
  static String? validateHolderName(String? value) {
    if (value == null || value.trim().isEmpty) return "Account holder name is required";
    if (value.trim().length < 3) return "Name must be at least 3 characters";
    
    // Security Upgrade: Block random special characters (only allow letters, spaces, dots, &, -, ')
    final nameRegex = RegExp(r"^[a-zA-Z\s\.\&\-\']+$");
    if (!nameRegex.hasMatch(value.trim())) return "Name contains invalid characters";
  ...
  static String? validateAccountNumber(String? value) {
    ...
    if (sanitizedValue.length < 6 || sanitizedValue.length > 18) return "Invalid account number length";
